package com.lxn.utilone.util.status;
...
/**
  ...
  *  功能描述: 根据别人的设置状态栏的显示状态的工具类 参考demo:https://github.com/laobie   主题目前为Theme.AppCompat.Light.DarkActionBar
 *             具体的功能包括 1:给状态栏着色(可以设置透明度) setColor
 *                           2：设置状态栏为全透明 setTransparent
 *
  *   作 者:  李晓楠
  *   时 间： 2016/10/19 16:30 
 */
public class StatusBarUtil {
    ...
    public static void setColor(Activity activity, @ColorInt int color, int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            //decorView  根布局是个Framelayout 布局
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            Log.i("lixiaonan","decorView.getClass()===="+decorView.getClass());
            int count = decorView.getChildCount();
            Log.i("lixiaonan","加之前的===="+decorView.getChildCount());
            Log.i("lixiaonan","decorView.getChildAt(0).getClass()的===="+decorView.getChildAt(0).getClass());

            if (count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
                decorView.getChildAt(count - 1).setBackgroundColor(calculateStatusColor(color, statusBarAlpha));
            } else {
                //给他添加一个和状态栏高度一样的view 背景色为传入的颜色值 这样因为此时状态栏是透明的 状态栏和这个view一重合就可以看成有色的状态栏
                StatusBarView statusView = createStatusBarView(activity, color, statusBarAlpha);
                decorView.addView(statusView);
            }
            Log.i("lixiaonan","加之后的===="+decorView.getChildCount());
            //设置自己写的根布局参数 为状态栏预留出控件防止重叠
            setRootView(activity);
        }
    }
    /**
     * 设置自己写的根布局页面参数  eg:就这玩意 FitsSystemWindows 防止出现页面和状态栏重叠的情况
     */
    @TargetApi(Build.VERSION_CODES.KITKAT)
    private static void setRootView(Activity activity) {
        ViewGroup rootView = (ViewGroup) ((ViewGroup) activity.findViewById(android.R.id.content)).getChildAt(0);
        Log.i("lixiaonan","android.R.id.content)===="+rootView.getClass());
        Log.i("lixiaonan","android.R.id.content)=count==="+((ViewGroup) activity.findViewById(android.R.id.content)).getChildCount());
      ...
      /*  Window window = activity.getWindow();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
           /* int option = View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
            ...
            //5.0 以上的处理 表示此窗口负责绘制系统栏的背景的标志。
            activity.getWindow().addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            activity.getWindow().clearFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            //请求具有最小系统提供的后台保护的半透明导航栏。
